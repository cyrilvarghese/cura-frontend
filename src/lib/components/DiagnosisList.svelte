<script lang="ts">
    ...
    function updateDiagnosisStatus(id: string, newStatus: DiagnosisStatus) {
        if (newStatus === "primary") {
            diagnoses = diagnoses.map((d) => ({
                ...d,
                status: d.id === id ? newStatus : d.status === "primary" ? "differential" : d.status,
                isPrimary: d.id === id,
                isIrrelevant: false,
    ...
    function isDiagnosisStatus(value: string): value is DiagnosisStatus {
        return ['primary', 'secondary', 'differential', 'ruled-out'].includes(value);
    }
</script>
...
<div class="space-y-4">
    {#if diagnoses && diagnoses.length > 0}
            ...
            <div class="flex flex-col space-y-2 py-2 border-b">
                <div class="flex items-center space-x-3">
                    <div class="flex-1 flex items-center justify-between gap-4">
                        <Label
                            class="text-base flex-1 {diagnosis.status === 'ruled-out' ? 'line-through text-gray-400' : ''}"
                        >
                            {diagnosis.text}
                        </Label>
                        <Select.Root
                ...
                {#if diagnosis.status === "primary"}
                    <div class="ml-0 mt-2">
                        <Textarea
                            ...
                            oninput={(e) => updateJustification(diagnosis.id, e.currentTarget.value)}
                            class="min-h-[100px]"
                        />
                    </div>
                {/if}
        ...
        <div class="text-sm text-muted-foreground mt-2">
            <p>Select the status for each diagnosis using the dropdown menu. Provide justification for your primary diagnosis.</p>
        </div>
    {/if}
</div>
